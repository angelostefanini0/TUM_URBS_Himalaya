...
def setup_solver(optim, solver_name, logfile='solver.log'):
    """ """
    if solver_name == 'gurobi':
        ...
        optim.set_options("log={}".format(logfile))
    elif solver_name not in ('appsi_highs', 'highs'):
...
def run_scenario(input_files, Solver, timesteps, scenario, result_dir, dt,
    ...
    if not optim.available(False):
        raise RuntimeError("Solver '{}' is not available.".format(Solver))
    optim = setup_solver(optim, Solver, logfile=log_filename)
