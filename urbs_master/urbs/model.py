...
def create_model(data, dt=1, timesteps=None, objective='cost',
    ...
    m.e_co_curtailment = pyomo.Var(
        m.tm, m.com_tuples,
        within=pyomo.NonNegativeReals,
        doc='Unused production of demand commodities (MW) per timestep')
...
def res_vertex_rule(m, tm, stf, sit, com, com_type):
    ...
    if com in m.com_demand:
        try:
            ...
            pass
        # Allow intermittent production above demand to be curtailed.
        power_surplus -= m.e_co_curtailment[
            tm, stf, sit, com, com_type
        ]
...
def res_env_step_rule(m, tm, stf, sit, com, com_type):
    if com not in m.com_env:
        ...
        max_per_hour = m.commodity_dict['maxperhour'][
            (stf, sit, com, com_type)
        ]
        if math.isinf(max_per_hour):
            return pyomo.Constraint.Skip
        environmental_output = - commodity_balance(m, tm, stf, sit, com)
        return environmental_output <= m.dt * max_per_hour
...
def res_env_total_rule(m, stf, sit, com, com_type):
    if com not in m.com_env:
        ...
        max_total = m.commodity_dict['max'][(stf, sit, com, com_type)]
        ...
        return env_output_sum <= max_total
