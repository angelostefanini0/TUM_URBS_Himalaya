...
def read_input(input_files, year):
    ...
    directory = os.environ.get('URBS_JSON_DIR', default_directory)
...
def load_json_file(input_path):
    try:
        ...
        if getattr(exc, 'errno', None) == 22:
            raise RuntimeError(
                f'Cannot read {input_path}. If it is stored in OneDrive, '
                'mark the project folder as Always keep on this device and '
...
def load_commodity_json_to_dataframe(input_path):
    ...
    data_list = load_json_file(input_path)
